...
def show_reprojections(image1, image2, uncalibrated_1, uncalibrated_2, P1, P2, K, T, R, plot=True):
  ...
  """ YOUR CODE HERE
  """
  P1proj=np.zeros((len(P1),3))
  P2proj=np.zeros((len(P2), 3))
  for i in range(0,len(P1)):
    P1proj[i,:]=(K@(R@np.array([[P1[i][0]],[P1[i][1]],[P1[i][2]]])+T.reshape(3,1))).reshape(1,3)
  for i in range(0,len(P2)):
    P2proj[i,:] =(K@(np.linalg.inv(R)@np.array([[P2[i][0]],[P2[i][1]],[P2[i][2]]])+(-R.T@T).reshape(3,1))).reshape(1,3)
  """ END YOUR CODE
  """
